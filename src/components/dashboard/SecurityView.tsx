...
import { getSecurityCompliance, type SecurityComplianceResponse } from "@/lib/security";
...
const SecurityView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    void getSecurityCompliance()
      .then((s) => setCompliance(s))
      .catch(() => {
        // no-op
      });
  }, []);
      ...
      {compliance ? (
        <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/10 p-4">
          <div className="flex items-center justify-between">
            <p className="text-sm font-semibold tracking-tight text-emerald-100">Enterprise Security & SOC2 Readiness</p>
            <span className="text-xs px-2 py-1 rounded-lg border border-emerald-300/30 text-emerald-200">
              {compliance.soc2Readiness} · {compliance.score}%
            </span>
          </div>
          <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-2 text-[11px]">
            <span className="rounded-lg border border-white/10 bg-black/20 px-2 py-1">TLS: {compliance.controls.tlsEnabled ? "on" : "off"}</span>
            <span className="rounded-lg border border-white/10 bg-black/20 px-2 py-1">DB Encrypt: {compliance.controls.dbEncryptionEnabled ? "on" : "off"}</span>
            <span className="rounded-lg border border-white/10 bg-black/20 px-2 py-1">MFA: {compliance.controls.mfaRequired ? "required" : "optional"}</span>
            <span className="rounded-lg border border-white/10 bg-black/20 px-2 py-1">Secrets: {compliance.controls.secretsProvider}</span>
            <span className="rounded-lg border border-white/10 bg-black/20 px-2 py-1">mTLS: {compliance.controls.mTlsEnabled ? "on" : "off"}</span>
            <span className="rounded-lg border border-white/10 bg-black/20 px-2 py-1">Backups: {compliance.controls.backups.multiRegion ? "multi-region" : "single-region"}</span>
            <span className="rounded-lg border border-white/10 bg-black/20 px-2 py-1">Audit Log: {compliance.controls.immutableAuditEnabled ? "immutable" : "mutable"}</span>
            <span className="rounded-lg border border-white/10 bg-black/20 px-2 py-1">SSO: {compliance.ssoProviders.filter((p) => p.enabled).map((p) => p.id).join(", ") || "none"}</span>
          </div>
        </div>
      ) : null}
