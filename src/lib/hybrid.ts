...
export const hybridBrainApi = {
  ...
  /** Uses axios so JWT + tenant headers are sent (raw fetch did not, breaking this in dev). */
  migrations: async (limit = 50) => {
    const { data } = await api.get<{ migrations: WorkloadMigration[] }>(`/hybrid-brain/migrations?limit=${limit}`);
    return data;
  },
  migrate: (params: {
